...
export default function ReviewSystem({ unitId, userId, canDeleteAll = false }: ReviewSystemProps) {
    ...
    const checkEligibility = async () => {
      if (isMockDatabase) {
        ...
        const hasCompletedLease = leases.some((l: any) =>
          l.tenant_id === userId && l.unit_id === unitId &&
        ...
        try {
          ...
          const { data } = await supabase.rpc('can_review_unit', { p_user_id: userId, p_unit_id: unitId });
  ...
  const loadReviews = async () => {
    ...
    if (isMockDatabase) {
      ...
      setReviews(allReviews.filter((r: any) => r.unit_id === unitId));
    } else {
      try {
        const { createClient } = await import('@/utils/supabase/client');
        const supabase = createClient();
        const { data } = await supabase.from('reviews').select('*').eq('unit_id', unitId).order('created_at', { ascending: false });
        setReviews(data || []);
      } catch (e) {
  ...
  const submitReview = async () => {
    ...
    setSubmitting(true);
    const newReview: any = { user_id: userId, unit_id: unitId, rating, comment: comment.trim(), created_at: new Date().toISOString() };
    if (isMockDatabase) {
      const allReviews = JSON.parse(localStorage.getItem('ez_reviews') || '[]');
      ...
      try {
        const { createClient } = await import('@/utils/supabase/client');
        await (await createClient()).from('reviews').insert(newReview);
      } catch (e) { console.error('Submit review error:', e); }
    }
  ...
  const deleteReview = async (reviewId: string) => {
    if (isMockDatabase) {
      const allReviews = JSON.parse(localStorage.getItem('ez_reviews') || '[]');
      localStorage.setItem('ez_reviews', JSON.stringify(allReviews.filter((r: any) => r.id !== reviewId)));
    } else {
      try {
        const { createClient } = await import('@/utils/supabase/client');
        await (await createClient()).from('reviews').delete().eq('id', reviewId);
  ...
  const handleKeyDown = (e: React.KeyboardEvent, star: number) => {
    if (e.key === 'ArrowRight' || e.key === 'ArrowDown') {
      e.preventDefault(); const next = Math.min(5, star + 1); setRating(next); starsRef.current[next - 1]?.focus();
    } else if (e.key === 'ArrowLeft' || e.key === 'ArrowUp') {
      e.preventDefault(); const prev = Math.max(1, star - 1); setRating(prev); starsRef.current[prev - 1]?.focus();
    } else if (e.key === ' ' || e.key === 'Enter') {
      e.preventDefault(); setRating(rating === star ? 0 : star);
    }
  };
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <div style={{
        ...
        background: reviews.length > 0 ? 'var(--primary-light)' : 'var(--glass-bg)',
        border: '1px solid ' + (reviews.length > 0 ? 'var(--primary)' : 'var(--glass-border)'),
      }}>
        <div style={{
          fontSize: '1.8rem', fontWeight: 800,
          color: reviews.length > 0 ? 'var(--primary)' : 'var(--text-muted)',
        ...
        <div>
          <div style={{ display: 'flex', gap: 3, marginBottom: 3 }}>
            {[1, 2, 3, 4, 5].map(star => (
              <Star
                key={star} size={15}
                fill={star <= averageRating ? 'var(--warning)' : 'none'}
                color={star <= averageRating ? 'var(--warning)' : 'var(--text-muted)'}
        ...
        canReview ? (
          <button
            ...
            style={{
              ...
              background: showForm ? 'transparent' : 'var(--primary-light)',
          ...
          >
            <Star size={15} />
            {showForm ? (lang === 'zh' ? '取消' : 'Cancel') : (lang === 'zh' ? '发表评价' : 'Write a Review')}
          </button>
        ) : (
          <div style={{
      ...
      {showForm && (
        <div style={{
          ...
          <div style={{ marginBottom: 16 }}>
            <label style={{ display: 'block', fontSize: '0.82rem', color: 'var(--text-muted)', marginBottom: 10, fontWeight: 500 }}>
            ...
            <div style={{ display: 'flex', gap: 6, alignItems: 'center' }} role="radiogroup" aria-label={lang === 'zh' ? '评分选择' : 'Rating selection'}>
              {[1, 2, 3, 4, 5].map(star => {
                ...
                return (
                  <button
                    key={star} ref={el => { starsRef.current[star - 1] = el; }}
                    role="radio" aria-checked={rating === star} aria-label={`${star} ${ratingLabels[star]}`}
                  ...
                  >
                    <Star
                      size={28} fill={isActive ? starColor : 'none'} color={isActive ? starColor : 'var(--text-muted)'}
                      strokeWidth={isActive ? 0 : 1.5}
                      style={{ transition: 'all 0.2s ease', filter: isHovered ? `drop-shadow(0 0 8px ${starGlow})` : 'none' }}
                    />
                  </button>
                );
              })}
            </div>
            <div style={{ marginTop: 8, minHeight: 20, fontSize: '0.82rem', fontWeight: 600, color: displayRating > 0 ? starColor : 'transparent', transition: 'all 0.2s ease' }}>
          ...
          {/* Comment */}
          <div style={{ marginBottom: 16 }}>
            <label htmlFor="review-comment" style={{ display: 'block', fontSize: '0.82rem', color: 'var(--text-muted)', marginBottom: 8, fontWeight: 500 }}>
              {lang === 'zh' ? '评价内容（可选）' : 'Comment (optional)'}
            </label>
            <textarea
              id="review-comment" value={comment}
              ...
              style={{
                ...
                color: 'var(--text-h)', resize: 'vertical', fontSize: '0.85rem', lineHeight: 1.5,
                outline: 'none', transition: 'border-color 0.2s ease, box-shadow 0.2s ease', boxSizing: 'border-box',
              }}
          ...
          <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
            <button onClick={() => { setShowForm(false); setRating(0); setComment(''); }} style={{ padding: '10px 18px', borderRadius: 10, border: '1px solid var(--glass-border)', background: 'transparent', color: 'var(--text-muted)', cursor: 'pointer', fontSize: '0.85rem', fontWeight: 500, transition: 'all 0.15s ease' }}>
              {lang === 'zh' ? '取消' : 'Cancel'}
            </button>
            <button
              ...
              style={{
                ...
                display: 'flex', alignItems: 'center', gap: 8,
                transition: 'all 0.2s ease', opacity: rating > 0 ? 1 : 0.6,
                transform: submitting ? 'scale(0.98)' : 'scale(1)',
              }}
            >
              {submitting ? (
                <><Loader2 size={14} style={{ animation: 'spin 1s linear infinite' }} />{lang === 'zh' ? '提交中...' : 'Submitting...'}</>
              ) : (
                <><Send size={14} />{lang === 'zh' ? '提交' : 'Submit'}</>
              )}
            </button>
          </div>
        </div>
      ...
      {loading ? (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {[1, 2, 3].map(i => (
            <div key={i} style={{ padding: 14, borderRadius: 10, border: '1px solid var(--glass-border)', background: 'var(--glass-bg)' }}>
              <div style={{ display: 'flex', gap: 3, marginBottom: 8 }}>
                {[1, 2, 3, 4, 5].map(s => (
                  <div key={s} style={{ width: 14, height: 14, borderRadius: 3, background: 'var(--glass-border)', animation: `shimmer 1.5s ease-in-out infinite ${s * 0.05}s` }} />
                ))}
              </div>
              <div style={{ width: `${60 + i * 10}%`, height: 10, borderRadius: 5, background: 'var(--glass-border)', animation: 'shimmer 1.5s ease-in-out infinite 0.3s' }} />
            </div>
          ))}
        </div>
      ) : reviews.length === 0 ? (
        <div style={{
          textAlign: 'center', padding: 28, borderRadius: 12,
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {reviews.slice(0, MAX_VISIBLE_REVIEWS).map((review, i) => (
            <ReviewCard key={review.id} review={review} userId={userId} onDelete={deleteReview} lang={lang} index={i} canDeleteAll={canDeleteAll} />
          ))}
        ...
        >
          ...
          >
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px 20px', borderBottom: '1px solid var(--glass-border)' }}>
              ...
              <button
                onClick={() => setShowAllReviews(false)} aria-label={lang === 'zh' ? '关闭' : 'Close'}
                style={{ background: 'none', border: 'none', color: 'var(--text-muted)', cursor: 'pointer', padding: 6, borderRadius: 8, display: 'flex', transition: 'all 0.15s ease' }}
...
function ReviewCard({ review, userId, onDelete, lang, index = 0, canDeleteAll = false }: {
  review: Review; userId: string | null; onDelete: (id: string) => void; lang: string; index?: number; canDeleteAll?: boolean;
}) {
  const [confirmDelete, setConfirmDelete] = useState(false);
  const isOwner = userId === review.user_id;
  const showDelete = isOwner || canDeleteAll;
  const reviewStarColor = review.rating >= 4 ? 'var(--success)' : review.rating >= 3 ? 'var(--warning)' : 'var(--danger)';
        ...
        <div>
          <div style={{ display: 'flex', gap: 2, alignItems: 'center', marginBottom: 4 }}>
            {[1, 2, 3, 4, 5].map(star => (
              <Star
                key={star} size={13}
                fill={star <= review.rating ? reviewStarColor : 'none'}
                color={star <= review.rating ? reviewStarColor : 'var(--text-muted)'}
            ...
            {canDeleteAll && !isOwner && (
              <span style={{
                fontSize: '0.65rem', color: 'var(--text-muted)', marginLeft: 4,
                padding: '1px 5px', borderRadius: 4, background: 'var(--glass-border)',
              }}>
          ...
          confirmDelete ? (
            <div style={{ display: 'flex', gap: 4, alignItems: 'center' }}>
              <button
                ...
                style={{ background: 'var(--danger-light)', border: '1px solid var(--danger)', borderRadius: 6, padding: '3px 8px', cursor: 'pointer', color: 'var(--danger)', fontSize: '0.72rem', fontWeight: 600, transition: 'all 0.15s ease' }}
              >
                {lang === 'zh' ? '确认删除' : 'Confirm'}
              </button>
              <button onClick={() => setConfirmDelete(false)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-muted)', padding: 3, fontSize: '0.72rem' }}>
                {lang === 'zh' ? '取消' : 'Cancel'}
              </button>
            </div>
          ) : (

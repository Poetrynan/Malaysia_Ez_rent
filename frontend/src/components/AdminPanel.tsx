...
export default function AdminPanel({ adminRole: propAdminRole, defaultTab, activeTab, hideTabBar = false, onPendingCountsChange, onTabChange }: { adminRole: 'super_admin' | 'editor' | null; defaultTab?: 'dashboard' | 'properties' | 'leases' | 'admins' | 'feedback' | 'agent-reviews' | 'reviews' | 'profile'; activeTab?: 'dashboard' | 'properties' | 'leases' | 'admins' | 'feedback' | 'agent-reviews' | 'reviews' | 'profile'; hideTabBar?: boolean; onPendingCountsChange?: (leasesCount: number, feedbackCount: number, agentReviewsCount: number) => void; onTabChange?: (tab: string) => void; }) {
  ...
  const [adminImgModal, setAdminImgModal] = useState<string | null>(null);
  ...
  const fetchAdmins = async () => {
    try {
      if (!isLive) {
        ...
        setAdminList(admins.filter((a: { avatar_url?: string }) => !a.avatar_url?.startsWith('DELETED:')));
        return;
      }
  ...
  const getAdminInitial = (admin: { display_name?: string; email?: string }) => {
    const name = (admin.display_name || admin.email || 'A').trim();
                  ...
                  >
                    ...
                    <div style={{ padding: '20px 24px 24px', display: 'flex', flexDirection: 'column', gap: 20 }}>
                      ...
                      {!isSuper && (
                        ...
                        >
                          <Trash2 size={15} />
                          {lang === 'zh' ? '移除该中介' : 'Remove this agent'}
                        </button>
                      )}
                    </div>
                  </div>
          ...
          {adminImgModal && (
              ...
              >
                <div style={{ position: 'relative', maxWidth: '90vw', maxHeight: '85vh' }}>
                  ...
                  <img src={adminImgModal} alt="Preview" style={{ maxWidth: '100%', maxHeight: '85vh', borderRadius: 10, objectFit: 'contain' }} />
                </div>
              </div>
            )}
                                ...
                                <div style={{
                                  ...
                                  <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                                    <span style={{
                                      ...
                                      {isAgent ? (lang === 'zh' ? '中介' : 'Agent') : (lang === 'zh' ? '租客' : 'Tenant')}
                                    </span>
                                  </div>
                                  <span>
          ...
          <div style={{
            ...
            {([
              ...
              return (
                <button
                  key={st.key}
                  onClick={() => { setReviewsSubTab(st.key); setReviewDeleteConfirm(null); }}
                  style={{
                    display: 'flex',
                    alignItems: 'center',
                    gap: 8,

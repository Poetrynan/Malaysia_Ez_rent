...
│   │   │   ├── upload/page.tsx         # 房源上传（4步分步表单 + 压缩 + Google Places + 中介收款码）
│   │   │   ├── feedback/page.tsx       # 工单消息 + 支付凭证审核（通过/驳回）
│   │   │   └── profile/page.tsx        # 个人资料 + 退出登录
│   │   ├── mt/                         # 租客手机端轻量工作站（自动 UA 分流）
│   │   │   ├── layout.tsx              # AuthProvider + 身份门禁 + useTenantDataLoader + MobileTenantShell
│   │   │   ├── listings/page.tsx       # 房源浏览（ListingsDataContext 共享缓存）
│   │   │   ├── lease/page.tsx          # 租约管理（账单支付弹窗 + 凭证上传 + 维修工单）
│   │   │   └── profile/page.tsx        # 个人资料 + 身份验证（TenantDataContext）
│   │   └── mobile-upload/[id]/page.tsx # anonymous evidence upload
│   ├── src/components/
│   │   ├── MobileShell.tsx             # 中介手机端底部 Tab 导航 + 语言/主题切换 + 电脑端提示
│   │   ├── MobileTenantShell.tsx       # 租客手机端底部 Tab 导航 + 语言/主题切换 + 电脑端提示
...
│   │   ├── useTenantDataLoader.ts      # shared data loader hook for mobile tenant workstation
...
- **Account deletion** — two paths, both via Service Role (`SUPABASE_SERVICE_ROLE_KEY`):
  - **Self-delete** (`deleteAccountAction` in `frontend/src/app/actions/deleteAccount.ts`, triggered from `AppTopbar`):
    - **Tenant Day 0**: Deletes `tenant_interests`, `maintenance_requests`, `user_notifications`, and `auth.users` immediately. Marks `public.users.avatar_url = 'DELETED:{iso}'`; passport/IC photos retained 7 days as legal evidence.
    - **Tenant Day 7+**: `cleanupExpiredDeletedAccountsAction()` dissociates `leases` / `agent_ratings` / `reviews` (`tenant_id` / `user_id` → `null`), deletes document images from Storage, then deletes `public.users`.
    - **Agent self-delete Day 0**: **Blocked** if the agent has any `active` lease on their `units`. Otherwise: deletes `user_notifications`, revokes `auth.users`, marks `users.avatar_url`, `admin_users.avatar_url`, and `agent_profiles.rejection_reason` with `DELETED:{iso}` (`verification_status` → `rejected`). REN tag / agency records retained 7 days.
    - **Agent self-delete Day 7+**: Purges `agent_profiles`, `admin_users`, `users`, and REN images from Storage. **Leases and payment records are never deleted** — they remain as the agent's business archive.
  - **Super-admin remove** (`deleteAgentBySuperAdminAction` in `frontend/src/app/actions/deleteAgentBySuperAdmin.ts`, triggered from AdminPanel): **immediate full purge** of `user_notifications`, `agent_profiles`, `admin_users`, `users`, REN Storage files, and `auth.users`. Also blocked when active leases exist. Does **not** use the 7-day retention window.
  - **Cleanup triggers**: (1) opportunistic run on each `deleteAccountAction`; (2) Vercel Cron `GET /api/cron/cleanup-expired-deleted-accounts` daily at 03:00 (`frontend/vercel.json`). Requires `CRON_SECRET` — without it, cron returns 401 (platform unaffected).
  - **UI**: `AppTopbar` shows role-aware deletion copy (tenant vs agent). `AdminPanel` hides `DELETED:`-marked agents from the admin list.

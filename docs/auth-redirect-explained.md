...
### 多语言与深色模式支持
* 登录页面（`/login`）现在完整接入了全局的 `ThemeProvider`，通过顶部右侧的语言与主题切换按钮，允许用户随时切换“中/英”语言和“深/浅”主题。
...
*文档更新：2026-05-25 · 含合租意向 RPC（014/015）、自行取消、缴租银行/微信/支付宝、禁止 iProperty、Memory vs Storage、Agent权限隔离与红点气泡通知、登录多语言与深色模式、房源列表多视图模式切换*
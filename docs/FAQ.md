...
| 删除对象 | 同步清理的内容 |
|----------|--------------|
| 房源（unit） | Storage 里的图片文件 + 关联租约/付款/意向级联删除 |
| 租约（lease） | 关联的 payment_records + 恢复 unit 状态为 available |

所有删除操作都有确认弹窗，防止误操作。

删除成功后，管理端列表会**立即**移除该行（乐观更新本地状态 + 后台与 Supabase 同步），无需手动刷新页面；租客端房源列表缓存也会同步刷新。
...
### Q: 租客身份验证有哪些要求？
**A:** 系统已统一实名验证规则（2026-06 更新）。

**先分清两条路径（UI 不一样）：**

| 场景 | 走哪个页面 |
|------|------------|
| 全新注册（从没账号） | `/register/tenant` — 注册向导（含邮箱验证、设密码） |
| 已有账号，登录后缺身份资料（老租客、Google 首次等） | `/profile` — 个人信息页里的「身份验证」区块 |

注册和登录后补资料是**两套界面**。但**进入租客端之后**，大家的「个人信息」都是同一个 `/profile` 页面（新租客资料已预填，老租客首次要补）。

**三层验证规则：**

**第一层：登录门禁（硬性）**
- 租客登录后若 `identity_type` 未设置，**只能访问 `/profile`**，其他页面（房源、租约、报修等）会被自动重定向到个人信息页。
- 适用入口：中间件、密码登录、Google OAuth 回调、沙盒模式客户端门禁。

**第二层：个人信息保存（硬性）**
- 在 `/profile` 保存资料时，必须选择身份类型并上传对应证件：
  - **马来西亚本地人**：12 位 IC 号 + 身份证正反面照片
  - **国际留学生**：护照号 + 护照照片页（学生证选填）
  - **其他外籍人士**：护照号 + 护照照片页（工作准证选填）

**第三层：表达租房意向（软性提醒）**
- 点「我要租」时，若证件信息仍不齐全，会弹出 **Modal** 提醒：中介审核时可能因资料不全而拒绝。
- 租客可选择「去完善资料」或「仍要提交意向」（不阻断操作）。

**新租客注册**（`/register/tenant`）与上述保存规则一致，证件为必传。

**证件压缩：** 上传时自动压缩（最大约 1200–1600px，JPEG 75–85%），存入 Storage `tenant-docs/` 目录。

**相关字段：** `identity_type`, `local_id_number`, `passport_number`, `ic_photo_front_url`, `ic_photo_back_url`, `passport_photo_url`, `student_card_url`, `work_permit_photo_url`
...
游客点击"我要租"或"加入合租"时，会看到"登录后即可..."引导提示。

### Q: 游客页面的房源列表有分页吗？

**A:** 有。游客模式每页显示 8 个房源（网格视图 4 列×2 行），有上一页/下一页和页码按钮。筛选条件变化时自动回到第 1 页。登录用户网格视图每页 10 个（5 列×2 行），列表视图每页 8 行。

### Q: 入住人数为什么不再随确认意向变化？

**A:** 之前的 Bug 是：中介点"确认意向"后，`tenant_interests.status` 变为 `'confirmed'`，入住人数立即+1。但"确认意向"只表示中介同意，不代表租客已入住。

修复后，入住人数基于**实际活跃租约**（`leases.status = 'active'`）计算。只有中介创建租约后，入住人数才会增加。

### Q: 身份验证进度条是什么？

**A:** 登录用户访问 `/listings` 时，页面顶部显示一条蓝色进度条（类似 YouTube/Google 的加载条），从左到右平滑填充。进度条下方显示"身份校验中..."文案。校验完成后：学生→房源列表，中介→管理后台。

---

*文档更新：2026-06-06 · 游客模式、分页、动态标题、入住人数修复、身份验证进度条*

---

### Q: Guest 页面为什么看不到侧边栏和顶栏？

**A:** Guest 页面（`/guest`）是纯浏览页面，不显示侧边栏、顶栏、注销按钮、数据库状态。无论是否登录，访问 `/guest` 都是干净的浏览界面。已登录用户会看到"进入系统"按钮跳转到 `/listings`。

---

### Q: 进度条还会闪烁吗？

**A:** 已修复。未登录用户访问 `/listings` 时不再显示进度条，直接跳转到 `/guest`。进度条只在真正检查登录状态时（`loading === true`）才显示。

---

### Q: 网格视图一排几个？列表视图几个？

**A:** 租客/中介端网格视图固定 5 列 × 2 行 = 每页 10 个。列表视图每页 8 个。Guest 端网格视图自适应 4 列。超过数量自动分页，切换视图模式时自动重置页码。

---

*文档更新：2026-06-06 · Guest 页面净化、进度条修复、网格分页*

---

### Q: 切换 tab 时房源列表为什么要等/会卡？现在怎么优化的？

**A:** 原先每次切到「房源列表」，React 会重新挂载整个 `PropertyListings` 组件，并重新向 Supabase 请求 `units`、`communities`、`admin_users` 等，页面先显示转圈再出卡片。
...
### Q: 这些性能优化会占用 Supabase / Vercel / Render 资源吗？

**A:** **主要占用浏览器内存**（缓存几百条房源数据，通常可忽略）。

| 服务 | 影响 |
|------|------|
| **Supabase** | 同一会话内重复访问读次数**减少**（有缓存就不每次打库） |
| **Vercel** | 几乎无变化（仍只托管前端页面） |
| **Render** | 无影响（AI 后端不参与房源列表加载） |

房源数据是浏览器直连 Supabase，不经过 Vercel 服务器或 Render。

---

### Q: 浏览器能「记住密码」吗？需要服务器开发吗？

**A:** **不需要服务器。** 登录页和注册页的邮箱/密码框已加标准 HTML 属性（`name`、`autocomplete`），浏览器原生密码管理器会：
- 登录/注册成功后提示「保存密码」
- 下次打开登录页自动填充账号密码

这是纯前端 HTML 语义，零后端改动。详见 `FUTURE_IMPROVEMENTS.md` #38。

---

### Q: 打开房源详情时评价加载慢吗？

**A:** 原先每条评价单独查一次 `users` 表（N+1，20 条评价 ≈ 21 次请求）。**已修复（2026-06-06）**：改为一次批量 `users.in('id', userIds)`，约 2 次请求。

---

*文档更新：2026-06-06 · Tab 切换性能、删房源即时 UI、浏览器密码、评价批量查询*

---

### Q: 为什么注册时突然不发验证码了？

**A:** **为了简化本地开发与测试部署（2026-06-09）**，平台临时绕过了注册阶段的 Resend 邮箱验证码发送和校验步骤。学生和中介在注册时填写的邮箱均默认通过激活，点击注册直接进入对应身份完成后续建账动作。如果生产部署需要启用，只需还原 `register/tenant/page.tsx` 与 `register/agent/page.tsx` 中屏蔽的 Resend 验证码校验状态即可。

---

### Q: 忘记密码怎么重设？老租客没有设置过密码怎么办？

**A:** **已于 2026-06-09 完整上线密码重置工作流**。
- 用户可在登录页 `/login` 点击「忘记密码？」链接，输入注册邮箱接收重置验证邮件。
- 邮件内的重置令牌会将用户引流至独立的安全重置页 `/reset-password`。在这里可以为账户重新设定高强度的密码，保存后自动根据角色分流跳转。
- **对于从旧版 Magic Link 迁移过来的、没有设置过密码且无法使用 Google 快捷登录的老租客**，这套流程也是标准的凭证重设路径，让他们能够顺畅地找回并重新绑定账户密码。

---

### Q: 为什么在 AI 助手问 USM 的租房，会显示 UTM / UKM 的房源？怎么解决的？

**A:** 
- **原因**：之前的 RAG 检索模型在计算小区的 Embedding（向量嵌入）源文本时，只包含了小区名和房源描述，**完全遗漏了关联大学名称**。导致用户问 "USM" 时，`Arte S` 和 `Centrio Avenue` 计算出的相似度得分仅在 `0.45` 左右，低于匹配阈值 `0.5`，系统误认为 0 检索结果，从而触发了备选大学降级兜底，召回了其他不相关学校的小区。
- **解决方法（2026-06-09）**：修改了 embedding 计算机制，将关联大学字段（`university_name`）并入文本向量化源字符，并针对主数据库 `malaysia_rental_master_database.json` 重新计算同步了全部 153 个小区的向量索引。修复后对 "USM" 的检索相似度升至 `0.6` 以上，精准且稳定地匹配对应的小区。

---

### Q: 切换卡片时，为什么地图定位依然保留在之前的位置（不更新）？怎么修复的？

**A:**
- **原因**：这是因为浏览器缓存了 `<iframe>` 地图元素。当用户点击切换卡片时，虽然地图 URL 发生了变化，但 React 默认只是就地更新 `<iframe>` 的属性（主要是 `src`），而浏览器的渲染引擎和缓存策略在 src 微小变化时未能强行触发重画，导致地图画面定格在之前的坐标定位上。
- **修复方法（2026-06-09）**：在前端 `MapAndCard.tsx` 里的 Google 地图 `<iframe>` 标签中加入了唯一键 `key={mapUrl}`。这样，每当卡片经纬度及参数合成的 `mapUrl` 发生变化时，React 都会立刻卸载旧的 iframe 并实例化并挂载一个全新的 iframe，彻底绕过了浏览器缓存，强制重绘当前小区的定位大头针。

---

*文档更新：2026-06-09 · 邮箱验证临时绕过、重置密码工作流、USM 向量检索增强、地图 iframe 缓存重绘修复*

...
**Google 未完成补资料的过期清理（2026-06-06）**：

| 项目 | 说明 |
|------|------|
| 对象 | Google 登录、**中间态**（`identity_type` 为空）、建号超过 30 分钟 |
| 动作 | 删除 `public.users` + `auth.users`（物理清除，非软删除） |
| 保护 | 邮箱注册租客、有租约/意向/报修的用户、中介账号不会被误删 |
| 注意 | 30 分钟从**首次 Google 建号**算起；仍在 `/profile` 填表但未保存，超时也会被清 |
| 触发 | ① 每次 Google 登录回调异步清理（主要）；② Vercel Cron 每天 04:00 兜底（Hobby 最短每天 1 次） |
| 部署 | 需跑迁移 `045`；`CRON_SECRET` 可暂不配置（定时任务不执行，**不影响平台使用**） |
...
### 9.0.3 AdminPanel 扩展"添加管理员"功能

**现有表单字段**（AdminPanel.tsx 第 186 行）：
```
newAdmin = { email, display_name, phone, whatsapp, wechat_id }
```

**需要新增字段**：
```
+ ren_number        -- REN 编号
+ ren_tag_image_url -- REN 执照照片（上传）
+ agency_name       -- 公司名称
```

**改动**：
- 扩展 `newAdmin` state，新增 `ren_number`、`ren_tag_image_url`、`agency_name` 字段
- `handleAddAdmin()` 中，插入 `admin_users` 时带上 REN 信息
- REN 照片使用现有 `compressImageFile` + `REN_TAG_PRESET` 压缩上传
- 上传到 Supabase Storage `unit-media` 桶，路径 `ren-tags/ren-tag-{timestamp}.jpg`
- 新增 `agent_profiles` 记录（status: 'approved'），关联到该用户
